const TYPO_CHARS = { 'a': 's', 'e': 'r', 'i': 'o', 'o': 'p', 'u': 'i', 't': 'y', 'n': 'm', 's': 'a', 'r': 'e' };
const SKIP_PATTERNS = [/^(ok|okay|k|kk|oke|oki|okok|hmm|hm)$/i, /^(yes|no|ya|yep|nope|nah|yoi|iya)$/i, /^(lol|lmao|haha|wkwk|wkwkwk|😂|💀|🔥|👍|👎)$/i, /^(nice|cool|bet|aight|damn|dope|sick)$/i, /^.{1,4}$/, /^<a?:\w+:\d+>$/];
...
export function splitResponse(content) {
  ...
  for (const sentence of sentences) {
    if (partCount >= 3 && Math.random() > 0.15) { currentPart += ' ' + sentence; continue; }
    if (currentPart && Math.random() < (0.4 - partCount * 0.1)) {
      parts.push(currentPart.trim());
      currentPart = sentence;
      partCount++;
    } else { currentPart += (currentPart ? ' ' : '') + sentence; }
  }
  if (currentPart.trim()) parts.push(currentPart.trim());
  return parts.length > 0 ? parts : [content];
}
export function addTypo(text, quirks) {
  if (quirks === 'light' || Math.random() > 0.08) return { text, hasTypo: false };
  ...
  if (TYPO_CHARS[char]) {
    ...
    words[idx] = typoWord;
    return { text: words.join(' '), hasTypo: true, correction: `*${word}` };
  }
...
export function addFiller(text, quirks) {
  ...
  const fillers = ['hmm ', 'oh ', 'ah ', 'eh ', 'uhh ', 'yoo ', 'wait '];
  const reactions = ['nice. ', 'damn. ', 'lol. ', 'bruh. ', 'huh. '];
  if (Math.random() < 0.3) return fillers[Math.floor(Math.random() * fillers.length)] + text.charAt(0).toLowerCase() + text.slice(1);
  if (Math.random() < 0.2) return reactions[Math.floor(Math.random() * reactions.length)] + text.charAt(0).toLowerCase() + text.slice(1);
  return text;
}
export function lowercasify(text) {
  if (Math.random() > 0.7) return text;
  return text.toLowerCase().replace(/(?:^|[.!?]\s+)([a-z])/g, (m, c) => Math.random() > 0.6 ? m : m.toLowerCase());
}
export default { shouldSkip, shouldSplit, splitResponse, addTypo, calculateDelay, addFiller, lowercasify };
