...
const TOPIC_KEYWORDS = {
  gaming: ['game', 'play', 'steam', 'valorant', 'minecraft', 'roblox', 'rank', 'match', 'gg', 'noob'],
  personal: ['feel', 'sad', 'happy', 'love', 'hate', 'friend', 'family', 'girlfriend', 'boyfriend', 'life'],
  work: ['work', 'job', 'boss', 'project', 'deadline', 'meeting', 'office', 'salary'],
  hobby: ['music', 'movie', 'anime', 'art', 'draw', 'cook', 'gym', 'sport', 'book'],
  tech: ['code', 'programming', 'python', 'javascript', 'server', 'api', 'bug', 'error']
};
...
export async function saveMemory(userId, content, sentiment = 0) {
  ...
  for (const topic of topics) {
    try {
      await db.query(
        `INSERT INTO bot_memory (user_id, topic, content, importance)
         VALUES ($1, $2, $3, $4)`,
        [userId, topic, content.substring(0, 500), importance]
      );
    } catch (error) {
...
export async function updateUserTopics(userId) {
  try {
    const result = await db.query(
      `SELECT topic, COUNT(*) as cnt FROM bot_memory 
       WHERE user_id = $1 
       GROUP BY topic 
       ORDER BY cnt DESC 
       LIMIT 5`,
    ...
    if (topics.length > 0) {
      await db.query(
        `UPDATE user_memory SET topics = $2 WHERE user_id = $1`,
        [userId, topics]
      );
    }
  } catch (error) {
    console.error('[Memory] Update topics error:', error.message);
  }
}
