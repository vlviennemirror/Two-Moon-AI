...
export async function getUser(userId) {
  const cached = cache.getUserCache(userId);
  ...
  if (result.rows.length > 0) {
    cache.setUserCache(userId, result.rows[0]);
...
export async function upsertUser(userId, displayName, preferredLang = null) {
  ...
  cache.setUserCache(userId, null);
  return getUser(userId);
}

export async function updateUserSummary(userId, summary) {
  await db.query(
    'UPDATE user_memory SET context_summary = $2 WHERE user_id = $1',
    [userId, summary]
  );
  cache.setUserCache(userId, null);
}
...
export default { getUser, upsertUser, updateUserSummary, getUserLang };
